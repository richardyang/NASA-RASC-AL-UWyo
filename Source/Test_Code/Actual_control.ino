...
void update_arm_servos(const std_msgs::UInt16MultiArray&  cmd_msg){
  ...
  arm_servos[2].write(cmd_msg.data[2]);  //elbow
  arm_servos[3].write(cmd_msg.data[3]);  //wrist
}
...
void update_drive_servos(const std_msgs::UInt16MultiArray& cmd_msg){
  ...
  publish("WRITE: " + String(0) + " - " + String(drive_servos[0].read()));
  publish("WRITE: " + String(1) + " - " + String(drive_servos[1].read()));
}

// Subscribe to rostopic "arm_cmd" and "drive_cmd"
ros::Subscriber<std_msgs::UInt16MultiArray> sub_arm("arm_cmd", update_arm_servos);
ros::Subscriber<std_msgs::UInt16MultiArray> sub_drive_steer("drive_steer", update_drive_servos);

void setup(){
  nh.initNode();
  nh.advertise(chatter);
  nh.subscribe(sub_arm);
  nh.subscribe(sub_drive_steer);
  ...
  // set all to 90
  drive_servos[0].write(90);
  drive_servos[1].write(90);
  arm_servos[0].write(90);
  arm_servos[1].write(90);
  arm_servos[2].write(90);
  arm_servos[3].write(90);
}
